...
unsigned int StartI2C(void)
{
	...
	Nop();
	while (I2C3CONbits.SEN);	//Wait for Start Condition
...
{
	return (!I2C3STATbits.ACKSTAT);		//Return Ack Status
}
...
unsigned int getI2C(void)
{
	...
	Nop();
	while(!I2C3STATbits.RBF);		//Wait for receive buffer to be full
...
unsigned int getsI2C(unsigned char *rdptr, unsigned char Length)
{
	while (Length --)
	{
		*rdptr++ = getI2C();		//get a single byte
		...
		if(I2C3STATbits.BCL)		//Test for Bus collision
		{
			return(-1);
		}
